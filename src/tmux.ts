...
export function buildHudCommand(sessionPath?: string): string {
  return `node ${shellQuote(hudPath)} --codex${sessionPath ? ` ${shellQuote(sessionPath)}` : ''} --watch`;
}

export function buildCodexCommand(realCodex: string, codexArgs: string[]): string {
...
export function run(argv: string[] = process.argv.slice(2), env: NodeJS.ProcessEnv = process.env): number {
  ...
  if (env.TMUX) {
    ...
    if (split.status !== 0) return split.status ?? 1;
    if (parsed.hudOnly) return 0;
